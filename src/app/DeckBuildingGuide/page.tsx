// Hello this is QQinfinity, your resident Top Hat lover.

// I am biting the bullet and writing the first strategy guide for this channel here because being informative is kind of my whole deal™️ (ask the other old members of the discord). 
...
// This guide will attempt to explain the basic mindset and attitude necessary to build a decent deck that will compliment the echo of your choice. Hopefully after reading this guide you will make better informed choices when adding cards and formulating strategy. While this guide will include a few sample builds, please DO NOT take it as gospel, and adjust the cards to your preferences and playstyle. 

// Also this is mostly dueling. Not forest.

// The Venn Diagram:
// Imagine a Venn Diagram (two circles that overlap). One circle is SPELLS, and the other circle is CREATURES, while the central intersection is MIXED. 

// Generally, Harry and Hermione echo will want a deck predominantly composed of Spells, Newt will want a deck full of CREATURES, and everyone else is a different degrees of MIXED where they lean closer to having more spells or having more creatures. 

// It is highly discouraged to make a harry/hermy deck with more than one or two creatures (which are usually either Three Headed Dog, or Skrewt, or Golden Snitch＊), same for Newt decks having more than one or two spells. 

// (Golden Snitch is currently unavailable in Global)

// 【continued......】 
// Image
// 🎩QQinfinity, Ansem with Top Hat
// OP
//  — 07/03/2023 12:53 PM
...
// On the other side of the spectrum, Newt's echo effect will generate a 1 mana cost creature after playing three creature cards. Again, to maximize his effect, less spell is better.
...
// Picking your cards:
// While it is possible to separate cards into categories like 'support' or 'damage dealing', etc. For the purpose of DECK BUILDING, I propose an alternative sorting system that will make it slightly easier to pick your cards.

// Your Main Focus/Central Card
// The One (or Two) card that will be central to your strategy.

// Your complimentary cards/support

// Four or five cards that will boost your central card or will give extra value to your central strategy.

// Counter-Picks/Tech Cards
...
//  — 07/03/2023 1:11 PM
// yEr a WIzaRD, heRRy
// Harry's echo grants damage and healing boost to all spells that are 3 cost or lower. Naturally, we include cheap spells. Your main combo are the following: Oppugno + Broom & Inflatus + Bewitched Snowball. These are the cards that deal the highest damage in Harry's arsenal.

// Due to the fact that Harry spams a lot of cheap spells, we can benefit from the discount effect of Three Headed Dog. Note that you should ONLY use the dog when its 0 cost. Not when its 3 cost, not when its 2, and not when its 1. Only use at 0 cost.

// With that in mind, here are a collection of cards that synergise well with Harry, written in the aforementioned three category format:

// ⭐Central Cards:
// Bewitched Snowball, Oppugno

// :plus: Supports:
// Inflatus or Incarcerous (for snowball)
// Broom (for Oppugno)
// Expulso (very useful for Harry after damage buff)
// Nebulus
// Stupefy
// Expelliarmus
// Episkey or Essence of Dittany
// Portkey
// Prior Incantanto (Note: copied 3 cost cards becoming 4 cost CAN benefit from echo buff. Have fun with double Snowball)
// Three Headed Dog (discount)
// Golden Snitch ＊(not yet in global; function includes extra mana for casting and movement cards for kiting)
// Manticore (not recommended, but you could use this if you desperately need more movement)
// Skrewt
...
// Priority, in order, Bewitched Snowball, Oppugno, Expulso, Broom, Stupefy (or Expelliarmus), everything else in the support card section.

// https://youtu.be/UdRphSFXxfg 
// Image
// YouTube
// minaly22
// Wow Gurl, lv16? how did you get 3 consecutive wins when you play li...
// Image
// 🎩QQinfinity, Ansem with Top Hat
// OP
//  — 07/03/2023 1:26 PM
// Among harry's choice of companions, there are TWO very important cards that you will wish to squeeze into the roster. Luna Lovegood Companion and Prof. McGonagall companion. Unfortuntately, neither Luna companion or Prof. McGonagall are available in global yet. But as they are integral to future Harry echo combos, we will talk about them for a bit.

// The moment you summon Prof. McGonagall, she will transfigure every unit on the map into a small critter. Transfigured units take extra damage, and are slowed (and unable to attack, if they are creature or companion units). This is essentially a screen wide time-out button. 

// While she is on the field, every time you use a card, she will transfigure a random unit on the field. Which means if there is only the enemy wizard, she will consistently transfigure them, constantly slowing them and let you deal extra damage.

// After you have cast 8 cards, she will summon a single piertotum locomotor statue.

// Ideally, you wait until 9 or 10 mana, put down Prof. McGonagall, and spam all your spells in quick succession. You can hide/protect her with Nebulus, prolonging her presence on the field and creating pressure through slow effect/summon statue.

// -----

// While Luna Lovegood companion is on the field, every time you cast a spell, she will summon a random creature of corresponding cost -1. For example, cast a 3 mana spell, she will summon a random 2 cost creature for you. After accumulating a total of 20 mana, she will summon a Thestral, which deals high damage in a small area. Again, protect her with Nebulus, and spam your spells after waiting for 10 or 9 mana. She will summon a swarm of creatures for you, and if enemy doesn't kill her quickly, will summon a hard hitting thestral.

// But for now....
// In global, we suggest using Lottie, Ron, Hagrid.
// Replace Lottie with Grawp or Frey Twins, or Cassandra. 
// 🎩QQinfinity, Ansem with Top Hat
// OP
//  — 07/03/2023 1:45 PM
// Hermione Granger, Minister of Magic, Queen of HPMA
// It is no exaggeration that Hermy echo has dominated (and continues to dominate) the HPMA meta with her simplistic yet powerful echo effect. 

// Her echo effect involves halving the most expensive card in hand and boosting it's card level by 1 (50% probability for level boost) after playing three spells. 

// And so the usual way to build her is to pick One very expensive spell, and bring a whole bunch of cheap spells to facilitate her echo buff. Some variations pick Two very expensive spells and let the echo effect buff them in rotation.

// However we will discuss the two most popular hermy deck, the Hermy Thunderstorm, and the Hermy Firework/Whizbang.
...
// Whizbang, for Whizbang


// :plus: Supports:
// Oppugno+Broom (solid damage and cheap spell)
// Expulso
// Nebulus
// Portkey (for thunderstorm)
// Prior Incantanto (if aiming for double thunderstorm)
// Incarcerous (whizbang component)
// Inflatus (whizbang component)
// Incendio or Sectum Sempra (your secondary source of dps)
// Three Headed Dog
// Stupefy
// Expelliarmus
// Side Along Apparition
// Essence of Dittany or Episkey
// Snowball
// Blast Ended Skrewt
// Golden Snitch＊

// :tasks: Techs: 
// Acromantula Venom (grounded creatures)
// Accio (snipe companions and dragon eggs)
// Atmosphere Charm (healing reduction, vs. hagrid and neville)
// Confringo (newt creature swarm)
// Crucio (ron or grawp or fiendfyre)
// Aquamenti (all of the above mentioned functions, but with less efficiency) 
// 🎩QQinfinity, Ansem with Top Hat
// OP
//  — 07/03/2023 2:00 PM
// Sample of Thunder Hermy and Whizbang Hermy.
// Again, Luna and Prof. Mcgonagall are very strong picks for her, but its not available yet.

// Lottie/Grawp/Frey Twins are all worth considering. Cassandra is helpful in dealing with creatures but bad against spells. Use Hermy companion as 1st if you want double thunderstorm.
// Ron as second companion, or Hermy.
// Hagrid as third. 
// Image
// Thunder Hermy brings Portkey? Why Portkey?
// You can throw the boot into enemy nebulus and break the fog. The portkey will grant vision, allowing Thunderstorm to hit things that are hiding inside. 

// Bewitched snowball follows a similar logic. The snowman with the aoe slow effect also grants vision into nebulus, but it is a lot less reliable than portkey since people can just dodge snowball, potentially letting it roll past the nebulus entirely.

// If you don't find snowball to be a good fit for your deck, replace Snowball for something more useful, like three headed dog.

// Proof that boot can pierce nebulus
// https://youtu.be/PliDf95F7mI 
// YouTube
// minaly22
// By Odin-bledore's Beard
// Image
// Sample of Whizbang
 
// Image
// Remember that your echo discount occurs after three spells. 

// So generally you'll be using: cheap, cheap, cheap spell, then play the buffed expensive spell.

// For thunderstorm, cheap cheap cheap (example, oppugno into broom, then expulso) to discount the thunderstorm in your hand. Then you play thunderstorm. If enemy has nebulus, you wait for 2 additional mana, play thunderstorm and immediately throw portkey into enemy nebulus. For the sake of optimization, cheap->cheap->portkey->thunderstorm (after waiting for mana to play both portkey and thunderstorm immediately).

// For whizbang, cheap cheap spell, then a control spell, then whizbang. (example: oppugno into broom, then wait 7 mana, incarcerous into discounted 4 mana whizbang).

// Additional footages of Thunder Hermy:

// https://youtu.be/SiCZcWDwt2I
// https://youtu.be/ssUWv5phr64 
// YouTube
// minaly22
// Thundering through Fiendfyre
// Image
// YouTube
// minaly22
// Wish I didn't miss that Confringo
// Image
// 🎩QQinfinity, Ansem with Top Hat
// OP
//  — 07/03/2023 2:17 PM
...
// :plus: Supports:
// Niffler
// Phoenix ＊ (this is complicated. we will explain later)
// Golden Snitch＊(not available)
// Manticore
// Matagot
// Pixie
...
// Spiders and Occamy is Just Bad™️. Stack of Monster Book of Monsters takes 7 mana but dies to a single cast of Incendio which is 4 mana, putting you at a 3 mana disadvantage. Please don't ever use stack of books with Newt.
...
// OP
//  — 07/03/2023 3:19 PM
// Snape: What is that Mysterious Ticking Noise™️? :SnapeReally:
// It's a time bomb!:VolCry: 💣  It's a time turner!
...
// :plus: Supports
// Oppugno + Broom 🧿 (IMPORTANT! will explain later)
// Atmosphere Charm
...
// Aquamenti
// Orb of Water
// Ventus (👎 bad card. bad damage, laughable effect of gently pushing away enemies. why push enemies when you can just kill? anyway, its multi hit, whatever)
// Monster Book (👎 frail, bad damage, it could attack multiple times very quickly, but use it to break nebulus, only use when necessary)
// Stack of Monster Book of Monsters (👎 Bad card, but Bella is literally the only person that could use it) 
...
// YouTube
// 小綱綱Kang
// 索命咒 一擊必殺!! 世界排名前50玩家 直播斬殺合集與牌組推薦 【哈利波特:魔法覺醒】小綱綱Kang (English/日本語 CC...
// Image
...
// putting in multiple creatures of the same cost (Fiendfyre+Dragon egg ; Piertotum Locomotor+Opal Eye＊)
// putting creatures that are not suitable for Hagrid in general (Opal Eye＊)
...
// Golden Snitch＊ (not available. same old)
// Pixies
// Matagot
...
// Harry here: 
// ⁠🧠strategies-and-guides⁠QQ's Introduction to Deck Build…
...
// "Monster Books are Bad actually" Rant™️ here:
// ⁠🧠strategies-and-guides⁠QQ's Introduction to Deck Build…
...
// "Level Differences Shouldn't Be Allowed To Exist" Rant™️ here:
// ⁠🧠strategies-and-guides⁠QQ's Introduction to Deck Build…
...
// Oh, and please direct any further discussion or questions into the ⁠✨charms-talk or other channel. If we start chatting here its only going to get longer and longer (it's already 75 posts here!) then navigation will be a nightmare.
...
// OP
//  — 07/16/2023 5:27 AM
// The Importance of SLAPPING🫲 THAT🫱 NIFFLER🖐️
